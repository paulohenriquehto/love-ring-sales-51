-- Inserir os 3 produtos base com SKUs únicos
...
WITH product_ant33 AS (
  SELECT id FROM public.products WHERE sku = 'ANT33-ALLMYLIFE'
),
sizes AS (
...
WITH product_ant08 AS (
  SELECT id FROM public.products WHERE sku = 'ANT08-ALLOFME'
),
sizes AS (
  SELECT generate_series(8, 36) AS size_num
),
...
WITH product_ant116 AS (
  SELECT id FROM public.products WHERE sku = 'ANT116-ALLWENEED'
),
sizes AS (
  SELECT generate_series(8, 36) AS size_num
),
...
WHERE sku IN ('ANT33-ALLMYLIFE', 'ANT08-ALLOFME', 'ANT116-ALLWENEED');