-- Desabilitar temporariamente o trigger de geração de SKU
ALTER TABLE public.product_variants DISABLE TRIGGER variant_sku_trigger;

-- Limpar todas as variantes dos produtos novos
...
INSERT INTO public.product_variants (product_id, size, color, price_adjustment, active, sku_variant)
SELECT 
  p.id,
  ...
  0,
  true,
  'AF01-PERFEITO2-' || LPAD(s.size_num::text, 2, '0') || '-DOURADO'
FROM product_af01 p
CROSS JOIN sizes s;
...
INSERT INTO public.product_variants (product_id, material_id, size, width, color, price_adjustment, active, sku_variant)
SELECT 
  p.id,
  ...
  w.price_adj,
  true,
  'ANT14-THOUSAND-' || LPAD(s.size_num::text, 2, '0') || '-' || UPPER(c.color) || '-' || w.width
...
SELECT 
  ...
  w.price_adj,
  true,
  'ANT110-AWAYFROMSUN-' || LPAD(s.size_num::text, 2, '0') || '-PRETO-' || w.width
FROM product_ant110 p
CROSS JOIN sizes s
CROSS JOIN widths w;

-- Reabilitar o trigger
ALTER TABLE public.product_variants ENABLE TRIGGER variant_sku_trigger;